...
namespace TLLib
{
    public class AddressBook1
    {
        ...
        public string AddressBook1Insert1(
            string FirstName,
            string LastName,
            string Email,
            string HomePhone,
        ...
        )
        {
            ...
            {
                var scon = new SqlConnection(connectionString);
                var cmd = new SqlCommand("usp_AddressBook1_Insert1", scon);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@FirstName", string.IsNullOrEmpty(FirstName) ? dbNULL : (object)FirstName);
                cmd.Parameters.AddWithValue("@LastName", string.IsNullOrEmpty(LastName) ? dbNULL : (object)LastName);
                cmd.Parameters.AddWithValue("@Email", string.IsNullOrEmpty(Email) ? dbNULL : (object)Email);
                ...
                cmd.Parameters.AddWithValue("@RoleName", string.IsNullOrEmpty(RoleName) ? dbNULL : (object)RoleName);
                SqlParameter errorCodeParam = new SqlParameter("@ErrorCode", null);
                SqlParameter productIDParam = new SqlParameter("@OutAddressBookID", null);
                ...
                if (errorCodeParam.Value.ToString() != "0")
                    throw new Exception("Stored Procedure 'usp_AddressBook1_Insert1' reported the ErrorCode : " + errorCodeParam.Value.ToString());

                return productIDParam.Value.ToString();
            }
            //catch (SqlException ex)
            //{
            //    throw new Exception(ex.Number.ToString());
